...
import { useValidation } from '../../Hooks/useValidationNonMultiplicative';
import ContinueButton from '../../Components/ContinueButton';
import ContornoModal from '../../Components/ContornoModal/NonMultiplicative';
...
function NonMultiplicativeComponent({ initialState }: HomeWrapperProps) {
  ...
  const [fonteNeutrons, setFonteNeutrons] = useState<string>((initialState?.result as ResultStateNonMultiplicative)?.fonteNeutrons?.join(';') || "");
  const [coeficientesDifusao, setCoeficientesDifusao] = useState<string>(initialState?.result?.coeficientesDifusao?.join(';') || "");
  const [choquesMacroscopicos, setChoquesMacroscopicos] = useState<string>((initialState?.result as ResultStateNonMultiplicative)?.choquesMacroscopicos?.join(';') || "");
  ...
  const [incidenciaDir, setIncidenciaDir] = useState<number>((initialState?.result as ResultStateNonMultiplicative)?.incidenciaDir || 0);
  const [incidenciaEsq, setIncidenciaEsq] = useState<number>((initialState?.result as ResultStateNonMultiplicative)?.incidenciaEsq || 0);
  ...
  useEffect(() => {
    if (initialState?.result) {
      const savedState = (initialState?.result as ResultStateNonMultiplicative);
  ...
  const onSubmit = (onError: (err: Error) => void) => {
    try {
      const newResult = runAll({
        ...
        advancedOptions,
        filterPoint
      }) as ResultStateNonMultiplicative;
...
export default NonMultiplicativeComponent;